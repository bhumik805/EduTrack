...
with open('StateWise_Enrollment2.csv') as file:
...
male_phd = list(map(lambda r: int(r[8]) if r[8] != '' else 0, M))
female_phd = list(map(lambda r: int(r[9]) if r[9] != '' else 0, M))
...
layout = go.Layout(barmode='stack', title='Enrollment of male and female in Diploma')
